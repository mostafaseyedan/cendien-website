...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeAppLogic() {
        ...
        const PROMPT_CONFIG = {
            ...
            requirements: { defaultText: "5. A list of Key Requirements (e.g., mandatory, highly desirable). If none are explicitly stated, please indicate that.", delimiterKey: "REQUIREMENTS" },
        ...
        const sectionInstructionsMap = {
            ...
            requirements: "5. A list of Key Requirements (e.g., mandatory, highly desirable). If none are explicitly stated, please indicate that.",
        ...
        function formatAndDisplayContentWithPrompt(parentElement, sectionKeySuffix, sectionPromptText, sectionContentText) {
            ...
            const displayContent = sectionContentText.includes('not found in AI response') && sectionInstructionsMap[sectionKeySuffix]
                ? `N/A - ${sectionInstructionsMap[sectionKeySuffix].split('. ')[1] || ""}`
                : sectionContentText;

            const contentParagraph = document.createElement('p');
            contentParagraph.textContent = displayContent;
            parentElement.appendChild(contentParagraph);
        }
            ...
            if (filteredAnalyses.length === 0) {
                ...
                filteredAnalyses.forEach(analysis => {
                    ...
                    dropdownTrigger.addEventListener('click', (e) => {
                        ...
                        // Calculate if the dropdown would go off-screen
                        const rect = dropdownTrigger.getBoundingClientRect();
                        const spaceBelow = window.innerHeight - rect.bottom;
                        const spaceAbove = rect.top;
                        const dropdownHeight = dropdownMenu.scrollHeight; // Use scrollHeight to get full content height

                        if (spaceBelow < dropdownHeight && spaceAbove > dropdownHeight) {
                            // Not enough space below, but enough space above
                            dropdownMenu.classList.add('align-top');
                        } else {
                            // Enough space below, or not enough above, so default to below
                            dropdownMenu.classList.remove('align-top');
                        }
